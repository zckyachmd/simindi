<?php
  ...
  if (!isset($_GET["id"])) {
      ...
      if (mysqli_num_rows($check_dvd) >= 1) {
          mysqli_query($connect, "DELETE FROM meminjam WHERE no_peminjam = '$id'");

          if (mysqli_affected_rows($connect) > 0) {
              $get_result = "Berhasil menghapus Peminjaman " . $row['no_peminjam'];
              $result = base64_encode($get_result);
              header("Location: index.php?result=$result");
              exit;
          } else {
              $get_result = "Gagal menghapus Peminjaman " . $row['no_peminjam'];
